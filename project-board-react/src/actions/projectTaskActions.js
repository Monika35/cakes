...
export const addProjectTask = (project_task, history) => async dispatch => {
    try {
        await axios.post("http://localhost:3333/api/board", project_task);
        history.push("/");
        dispatch({
            type: GET_ERRORS,
...
export const getBacklog = () => async dispatch => {
    const res = await axios.get("http://localhost:3333/api/board/all");
    dispatch({
...
export const deleteProjectTask = pt_id => async dispatch => {
    if (
        ...
        await axios.delete(`http://localhost:3333/api/board/${pt_id}`);
        dispatch({
